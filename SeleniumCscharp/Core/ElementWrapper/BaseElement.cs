...
namespace SeleniumCSharp.Core.ElementWrapper
{
    ...
    public class BaseElement
    {
        ...
        {
            DriverUtils.ExecuteScript("arguments[0].scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'start' });",
                    GetElement());
            Thread.Sleep(250);
            WaitForPositionNotChange();
        }
            ...
            try
            {
                ...
                wait.Until(webDriver =>
                {
                    if (GetElement() != null)
                    {
                        ...
                        {
                            var pOld = element.Location;
                            var pNew = element.Location;
        ...
        {
            List<IWebElement> eleList = GetElements();
            List<string> resultList = new List<string>();
            try
            {
                for (int i = 0; i < eleList.Count; i++)
                {
                    resultList.Add(eleList[i].Text);
                }
